...
generate_config() {
    ...
    cat > "$HOME/.config/claude-code/claude_desktop_config.json" << # Create optimized Claude instructions
create_claude_instructions() {
    print_status "Creating optimized Claude instructions file..."
...
\`\`\`
- Configure form fields appropriately
- Set up table columns with proper formatting
- Implement filters and search functionality
- Add bulk actions when relevant

### When Creating Models:
\`\`\`bash
php artisan make:model ModelName -mfs
\`\`\`
- Always include migration (-m), factory (-f), and seeder (-s)
...
- Custom Alpine components: \`resources/js/components/\`
- Blade views: \`resources/views/\`
- Tailwind customizations: \`resources/css/app.css\`

## Testing Approach
- Write feature tests for all new functionality
- Use Laravel's testing utilities (actingAs, etc.)
- Test Livewire components with Livewire testing tools
- Mock external services appropriately
- Use factories for test data generation

## Development Workflow
1. Create migration and model with relationships
2. Build Livewire components for frontend interaction
3. Create Filament resources for admin management
4. Style with Tailwind CSS utilities
5. Add Alpine.js for enhanced interactivity
6. Write comprehensive tests
7. Optimize performance and security

## Communication Style
- Provide complete, working code examples
- Explain Laravel-specific concepts when relevant
...
EOF
    ...
    print_success "Optimized Claude instructions created!"
}
{
  "mcpServers": {
    "context7": {
...
EOF
    ...
    if grep -q "barryvdh/laravel-debugbar" composer.json 2>/dev/null; then
        cat >> "$HOME/.config/claude-code/claude_desktop_config.json.tmp" << 'EOF'
    "debugbar": {
      "command": "npx",
